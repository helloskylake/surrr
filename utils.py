...
def parse_ts(ts, env, action=None, is_joint=False, is_bi=False):
    ...
    if is_joint:
        ...
        if isinstance(ts, dict):
            new_ts.reward = 0
            if is_bi:
                new_ts.observation['qpos'] = env.psm1.get_current_joint_position() + [-0.8] + env.psm2.get_current_joint_position() + [-0.8]
            else:
                new_ts.observation['qpos'] = env.psm1.get_current_joint_position() + [0.5]  
        else:
        ...
        if isinstance(ts, dict):
            ...
            new_ts.reward = env.get_reward()
            new_ts.observation['qpos'] = ts[0]['observation'][:state_dim]  
            new_ts.observation['action'] = action  # .tolist()
    ecm_img, mask = env.ecm.render_image(640, 480)  
    front_img, front_mask = env.ecm.render_image_front(640, 480)  
    top_img, top_mask = env.ecm.render_image_top(640, 480)  
...
def parse_ts_abs(ts, env, action=None, is_ee=False):
    ...
    if is_ee:
        ...
        if isinstance(ts, dict):
            new_ts.reward = -1
            new_ts.observation['qpos'] = env.psm1.get_current_joint_position() + ts['observation'][6]  
        else:
            new_ts.reward = ts[1]
        ...
        if isinstance(ts, dict):
            ...
            new_ts.reward = ts[1]
            new_ts.observation['qpos'] = ts[0]['observation'][:7]  
            new_ts.observation['action'] = action  # .tolist()
    ecm_img, mask = env.ecm.render_image(640, 480)  
    front_img, front_mask = env.ecm.render_image_front(640, 480)  
    top_img, top_mask = env.ecm.render_image_top(640, 480)  
